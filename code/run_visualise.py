...
parser.add_argument('--initial_model', type=str,
                    default="data/syncnet.model", help='')
parser.add_argument('--batch_size', type=int, default='20', help='')
parser.add_argument('--vshift', type=int, default='15', help='')
parser.add_argument('--data_dir', type=str,
                    default='/mnt/hdd1/krdemo4', help='')
...
for ii, track in enumerate(tracks):
    ...
    for ij, frame in enumerate(track[0][0].tolist()):
        faces[frame].append([ii, fdist_mf[ij], track[1][0]
                             [ij], track[1][1][ij], track[1][2][ij]])
...
vOut = cv2.VideoWriter(os.path.join(
    opt.avi_dir, opt.reference, 'video_only.avi'), fourcc, cap.get(5), (fw, fh))
...
command = ("ffmpeg -y -i %s -ac 1 -vn -acodec pcm_s16le -ar 16000 %s -loglevel quiet" % (os.path.join(opt.avi_dir,
                                                                                                      opt.reference, 'video.avi'), os.path.join(opt.avi_dir, opt.reference, 'audio_only.avi')))
output = subprocess.call(command, shell=True, stdout=None)

# ========== COMBINE AUDIO AND VIDEO FILES ==========

command = ("ffmpeg -y -i %s -i %s -c:v copy -c:a copy %s -loglevel quiet" % (os.path.join(opt.avi_dir, opt.reference, 'video_only.avi'),
                                                                             os.path.join(opt.avi_dir, opt.reference, 'audio_only.avi'), os.path.join(opt.avi_dir, opt.reference, 'video_out.avi')))  # -async 1
output = subprocess.call(command, shell=True, stdout=None)
