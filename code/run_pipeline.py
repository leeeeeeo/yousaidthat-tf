...
parser.add_argument('--data_dir', type=str,
                    default='/dev/shm', help='Output direcotry')
...
parser.add_argument('--crop_scale', type=float,
                    default=0.5, help='Scale bounding box')
parser.add_argument('--min_track', type=int, default=5,
                    help='Minimum facetrack duration')
...
def crop_video(opt, track, cropfile):
    ...
    cap = cv2.VideoCapture(os.path.join(
        opt.avi_dir, opt.reference, 'video.avi'))
    ...
    command = ("ffmpeg -y -i %s -ac 1 -vn -acodec pcm_s16le -ar 16000 -ss %.3f -to %.3f %s -loglevel quiet" %
    ...
    command = ("ffmpeg -y -i %st.avi -i %s -c:v copy -c:a copy %s.avi -loglevel quiet" %
            ...
            while True:
                ...
                for index in range(0, len(score)):
                    if score[index] > MIN_CONF:
                        dets[-1].append([frame_num, boxes[0]
                                         [index].tolist(), score[index]])

                print('%s-%05d; %d dets; %.2f Hz' % (os.path.join(opt.avi_dir,
                                                                  opt.reference, 'video.avi'), frame_num, len(dets[-1]), (1/elapsed_time)))
                frame_num += 1
...
def scene_detect(opt):
    ...
    video_framerate, frames_read = scenedetect.detect_scenes_file(os.path.join(
        opt.avi_dir, opt.reference, 'video.avi'), scene_list, detector_list)
    ...
    print('%s - scenes detected %d from %d frames' % (os.path.join(opt.avi_dir,
                                                                   opt.reference, 'video.avi'), len(scene_list), frames_read))
...
command = ("ffmpeg -y -i %s -qscale:v 4 -async 1 -r 25 -deinterlace %s -loglevel quiet" %
           (opt.videofile, os.path.join(opt.avi_dir, opt.reference, 'video.avi')))  # -async 1
output = subprocess.call(command, shell=True, stdout=None)
...
for ii, track in enumerate(alltracks):
    ...
    vidtracks.append(crop_video(opt, track, os.path.join(
        opt.crop_dir, opt.reference, '%05d' % ii)))
