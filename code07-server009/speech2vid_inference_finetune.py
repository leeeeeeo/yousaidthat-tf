...
class Speech2Vid:
    def __init__(self, checkpoint_dir=None):
        ...
        self.v201_param_dict,self.v201_param_list=self.read_v201_param()
        print 'config loaded'
    ...
    def bn_layer(self, bottom, name):
        with tf.variable_scope(name,reuse=tf.AUTO_REUSE):
            ...
            if name in self.v201_param_list:
                ...
                return tf.layers.batch_normalization(bottom,axis=-1,epsilon=1e-4,gamma_initializer=gamma_init,beta_initializer=beta_init)
            else:
                print '{} not using pre-trained parameter'.format(name)
    ...
    def read_v201_param(self):
        ...
        for i in range(0, v201_param['v201_param_cell'].shape[1]):
            ...
            if not key.split('.')[0] in v201_param_list:
                if key.split('.')[0][0:2]=='co' or key.split('.')[0][0:2]=='fc' or key.split('.')[0][0:2]=='bn':
